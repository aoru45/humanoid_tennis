...
class HighLevelTennisCommand(
    ...
    RACKET_CENTER_OFFSET = (0.1035, -0.00325, 0.46277)
    ...
    def __init__(
        ...
        self.pre_hit_bounce_count = torch.zeros(self.num_envs, dtype=torch.int32, device=self.device)
        self.pre_hit_ball_court_contact_latched = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ...
        self.fail_second_bounce = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        ...
        self.pre_hit_bounce_event = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
        self.pre_hit_second_bounce_event = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
