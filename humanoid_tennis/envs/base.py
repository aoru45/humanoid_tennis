...
import humanoid_tennis.envs.mdp as mdp
import humanoid_tennis.utils.symmetry as symmetry_utils
...
class _ViserDebugDraw:
    ...
    def vector(self, starts, vectors, color=(1.0, 0.0, 0.0, 1.0), width: float | None = None):
        ...
        rgba = self._clamp_color(color)
        radius = (width if width is not None else self._arrow_width()) * 0.5
        for start, vec in zip(s, v):
            end = start + vec
            self._scene.add_cylinder(start, end, radius=radius, color=rgba)

    def ghost(self, qpos, model, alpha: float = 0.35, labels=None):
        q = self._to_numpy(qpos)
        if q.ndim == 1:
            q = q[None, :]
        if labels is not None and len(labels) != q.shape[0]:
            raise ValueError("ghost(): labels length must match batch size.")
        ghost_alpha = float(max(0.0, min(1.0, alpha)))
        for i, q_i in enumerate(q):
            label = None if labels is None else str(labels[i])
            self._scene.add_ghost_mesh(qpos=q_i, model=model, alpha=ghost_alpha, label=label)
...
class _Env(EnvBase):
    ...
    def __init__(self, cfg):
        self.cfg = cfg
        humanoid_tennis._ENVS = cfg.num_envs
        self.backend = humanoid_tennis.get_backend()
        self.device = f"cuda:{humanoid_tennis.get_local_rank()}"
    ...
    def _step(self, tensordict: TensorDictBase) -> TensorDictBase:
        ...
        # Compute termination before observation so catastrophic states that are
        # already marked done do not crash observation computation.
        ...
        # update observation
        self._obs_skip_mask = done.squeeze(-1)
        try:
    ...
    def _update_viewer(self, force_sync: bool = False) -> None:
        ...
        try:
            ...
            # Compatibility fallback for certain mjlab/viser versions where
            # arrow debug handles can be None during sync.
            if "batched_scales" in str(exc):
                try:
                    self._viser_scene.debug_visualization_enabled = False
                    self._viser_scene.update(wp_data)
                    print("[WARN] Disabled Viser debug visualization due to arrow sync issue.")
                except Exception:
                    pass
            else:
                raise
