...
class HighLevelTennisCommand(
    ...
    @staticmethod
    def _racket_body_guard_radius(body_name: str) -> float:
        name = str(body_name).lower()
        if ("head" in name) or ("torso" in name) or ("pelvis" in name) or ("waist" in name):
            return 0.18
        if any(k in name for k in ("hip", "thigh", "shin", "ankle", "foot", "linkage")):
            return 0.14
    ...
    def __init__(
        ...
        self.enable_racket_body_proximity_guard = bool(cfg.enable_racket_body_proximity_guard)
        self.racket_body_proximity_margin = max(0.0, float(cfg.racket_body_proximity_margin))
        ...
        self.racket_body_guard_body_ids = torch.zeros((0,), dtype=torch.long, device=self.device)
        self.racket_body_guard_body_radii = torch.zeros((0,), dtype=torch.float32, device=self.device)
        if self.enable_racket_body_proximity_guard:
            body_ids, body_names = self.asset.find_bodies(".*")
            guard_ids: list[int] = []
            guard_radii: list[float] = []
            for body_id, body_name in zip(body_ids, body_names):
                body_id_i = int(body_id)
                body_name_s = str(body_name)
                if body_id_i == self.racket_body_id:
                    continue
                low = body_name_s.lower()
                # Exclude only the mounting hand branch; monitor the rest of body.
                if "right_hand" in low:
                    continue
                guard_ids.append(body_id_i)
                guard_radii.append(self._racket_body_guard_radius(body_name_s) + self.racket_body_proximity_margin)
            if len(guard_ids) > 0:
                self.racket_body_guard_body_ids = torch.tensor(guard_ids, dtype=torch.long, device=self.device)
                self.racket_body_guard_body_radii = torch.tensor(guard_radii, dtype=torch.float32, device=self.device)
