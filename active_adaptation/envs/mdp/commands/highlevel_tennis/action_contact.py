...
class HighLevelTennisActionContactMixin:
    ...
    def _update_contact_events(self) -> None:
        ...
        # Proximity-based fallback guard for racket-vs-body penetration detection.
        # This catches severe visual interpenetration even when physical self-collision
        # between racket/body geoms is disabled.
        guard_ids = getattr(self, "racket_body_guard_body_ids", None)
        guard_radii = getattr(self, "racket_body_guard_body_radii", None)
        if (
            guard_ids is not None
            and guard_radii is not None
            and isinstance(guard_ids, torch.Tensor)
            and isinstance(guard_radii, torch.Tensor)
            and guard_ids.numel() > 0
            and bool(self.enable_racket_body_proximity_guard)
        ):
            racket_pos_w, _ = self._racket_state_w()
            body_pos_w = self.asset.data.body_link_pos_w.index_select(dim=1, index=guard_ids)
            diff = body_pos_w - racket_pos_w.unsqueeze(1)
            dist_sq = diff.square().sum(dim=-1)
            rad_sq = guard_radii.unsqueeze(0).square()
            proximity_hit = (dist_sq <= rad_sq).any(dim=-1)
            self.racket_body_contact[:] = self.racket_body_contact | proximity_hit
