...
def _get_racket_spec() -> mujoco.MjSpec:
    # Load racket-augmented robot directly from XML to avoid runtime spec surgery.
    spec = mujoco.MjSpec.from_file(str(G1_RACKET_XML))
...
# Enable racket collider only against tennis ball, while avoiding interactions with
# robot/terrain geoms that can destabilize locomotion.
G1_RACKET_BALL_COLLISION = CollisionCfg(
    geom_names_expr=("tennis_racket_collision",),
    contype=RACKET_COLLISION_CONTYPE,
    conaffinity=RACKET_COLLISION_CONAFFINITY,
...
G1_RACKET_CFG = EntityCfg(
    init_state=G1_INIT_STATE,
    collisions=(FULL_COLLISION, G1_RACKET_BALL_COLLISION),
